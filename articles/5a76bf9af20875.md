...
:::message
vimの実行中に何かを評価して実行するという点では`@=`( [`:help @=`](https://vim-jp.org/vimdoc-ja/change.html#@=) )も似ています。
マクロ実行の`@a`とかと似ていますが、こちらはExpressionレジスタと言われているように式を評価して実行する事ができます。
<!-- 式の評価結果を自動挿入してくれて便利(venry) -->
`@=getline('.')->split('\zs')->map({_, v -> "- [ ] " . v })`
↑例えばこんな感じに記述してやると、上のスクリプトでの編集と同じ変更を実行する事ができます。
後ほど供述するmethod記法を使って、様々な関数を組合せて式を記述する感覚ですね。
ちなみに、この評価時にも自作関数を呼び出せるので、うまく利用できると面白そう。
